...
class TongHuaShunCrawler(BaseCrawler):
    ...
    async def crawl(self) -> List[Dict]:
        """爬取涨停数据"""
        try:
            # 方法1: 尝试涨停池API（最稳定，但涨停原因经常为空）
            data = await self._crawl_api()
            if data:
                logger.info(f"[{self.name}] Crawled {len(data)} limit up stocks via limit_up_pool API")
                # 检查涨停原因是否完整
                missing_reason_count = sum(1 for d in data if not d.get("limit_up_reason"))
                if missing_reason_count > len(data) * 0.3:  # 超过30%缺失则补充
                    logger.info(f"[{self.name}] {missing_reason_count}/{len(data)} stocks missing reason, trying to enrich...")
                    data = await self._enrich_reasons_from_wencai(data)
                return data
    ...
    async def _enrich_reasons_from_wencai(self, data: List[Dict]) -> List[Dict]:
        """用问财API补充涨停原因"""
        try:
            # 尝试问财搜索获取涨停原因
            wencai_data = await self._crawl_wencai_search()
            if not wencai_data:
                wencai_data = await self._crawl_wencai()
            
            if wencai_data:
                # 构建股票代码到涨停原因的映射
                reason_map = {}
                for item in wencai_data:
                ...
                for item in data:
                    code = item.get("stock_code", "")
                    if not item.get("limit_up_reason") and code in reason_map:
                        item["limit_up_reason"] = reason_map[code]
                        item["reason_category"] = self._classify_reason(reason_map[code])
                        enriched += 1
                
                if enriched:
                    logger.info(f"[{self.name}] Enriched {enriched} stocks with reasons from Wencai")
            
            return data
        except Exception as e:
            logger.warning(f"[{self.name}] Enrich reasons error: {e}")
            return data
