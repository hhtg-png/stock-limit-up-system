...
class DailyAnalysisRecord(Base):
    ...
    intraday_auto_result = Column(JSON, default=dict, nullable=False, comment="盘中自动分析结果")
    intraday_manual_overrides = Column(JSON, default=dict, nullable=False, comment="盘中人工覆盖内容")
    intraday_calc_version = Column(Integer, default=0, nullable=False, comment="盘中计算版本")
    intraday_data_status = Column(String(20), default="empty", nullable=False, comment="盘中数据状态")
    intraday_generated_at = Column(DateTime, comment="盘中生成时间")
    created_at = Column(DateTime, default=datetime.now, nullable=False, comment="创建时间")
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now, nullable=False, comment="更新时间")
