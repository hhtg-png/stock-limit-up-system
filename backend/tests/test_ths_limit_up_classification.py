...
class ThsLimitUpClassificationServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rebuild_ai_cache_for_historical_date_uses_database_not_realtime(self):
        ...
        async with Session() as session:
            ...
            session.add(
                LimitUpRecord(
                    stock_id=stock.id,
                    trade_date=trade_date,
                    first_limit_up_time=datetime(2026, 6, 15, 9, 32, 0),
                    final_seal_time=datetime(2026, 6, 15, 9, 32, 0),
                    limit_up_reason="PCB铜箔+AI电源",
                    continuous_limit_up_days=1,
                    open_count=0,
                    is_final_sealed=True,
        ...
        ai_client = FakeAiClassificationClient(
            [{"stock_code": "600001", "plate_name": "PCB铜箔", "confidence": 0.95}]
        )
        service = ThsLimitUpClassificationService(
            realtime_service=FailingRealtimeLimitUpService(),
            ai_classification_client=ai_client,
        ...
        with patch("app.services.ths_limit_up_classification_service.async_session_maker", Session):
            await service.rebuild_ai_classification_cache(trade_date)
        ...
        self.assertEqual(ai_client.calls, [(trade_date, ["600001"])])
        self.assertEqual(len(cached), 1)
        self.assertEqual(cached[0].trade_date, trade_date)
        self.assertEqual(cached[0].classifications_json["classifications"][0]["plate_name"], "PCB铜箔")
