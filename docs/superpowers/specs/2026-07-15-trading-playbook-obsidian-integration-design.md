...
- `attempt_no`、`next_attempt_at`、`last_error`、`exported_at`、`created_at`、`updated_at`。
...
- 新增低频补偿任务，按 `next_attempt_at` 扫描 `pending` 和 `failed` 快照。
- 重试采用 1 分钟、5 分钟、15 分钟的退避上限，失败快照不会自动删除。
- Vault 禁用或未配置时状态为 `paused`，不增加失败次数；启用后自动恢复。
- 服务启动补跑会重新计算最近交易日的可变索引，并继续未完成的不可变快照。
