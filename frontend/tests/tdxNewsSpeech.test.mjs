...
assert.doesNotMatch(limitUp, /getTdxNews|realtimeNewsItems|NEWS_SNAPSHOT_LIMIT/, 'pure limit-up live plugin should not own aggregate news speech')
...
assert.match(composite, /NEWS_SNAPSHOT_LIMIT = 20/, 'composite plugin should keep the aggregate news snapshot small')
assert.match(composite, /getTdxNews\(\{\s*limit:\s*NEWS_SNAPSHOT_LIMIT\s*\}\)/, 'composite plugin should load a small aggregate news snapshot')
assert.match(composite, /watch\(\s*realtimeNewsItems/, 'composite plugin should listen to realtime aggregate news')
