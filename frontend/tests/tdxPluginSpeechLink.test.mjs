...
assert.match(limitUp, /enqueuePluginSpeech\('播报已开启'[\s\S]*urgent:\s*true/, 'pure limit-up voice switch should speak a low-latency enable check from the user gesture')
...
assert.match(composite, /enqueuePluginSpeech\('播报已开启'[\s\S]*urgent:\s*true/, 'composite voice switch should speak a low-latency enable check from the user gesture')
