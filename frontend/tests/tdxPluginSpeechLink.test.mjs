...
assert.doesNotMatch(speech, /mode:\s*'web-speech'/, 'urgent plugin speech should not bypass the TDX audio fallback')
assert.match(speech, /queueSpeechItem\(text,\s*\{\s*urgent:\s*true\s*\}\)/, 'urgent plugin speech should keep queue priority while using the normal fallback chain')
...
assert.match(speech, /primeTargetTtsAudio/, 'speech unlock should silently warm the audio element from the user gesture')
