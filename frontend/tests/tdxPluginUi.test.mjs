...
assert.match(limitUp, /handleStockClick\(item\)/, 'pure limit-up stock click should trigger the TDX stock link')
assert.doesNotMatch(limitUp, /embedded-move-panel|move-panel-resizer|getTdxStockMove/, 'pure limit-up plugin should not embed stock move analysis')
...
assert.match(composite, /embedded-move-panel/, 'composite plugin should embed stock move analysis in the lower panel')
assert.match(composite, /getTdxStockMove/, 'composite stock move panel should load stock move analysis data')
assert.match(composite, /handleStockClick\(item\)/, 'composite stock click should update analysis and trigger stock linking')
assert.match(composite, /move-panel-resizer/, 'composite stock move panel should have a vertical resize handle')
assert.match(composite, /@pointerdown="startMovePanelResize"/, 'composite stock move panel should resize by pointer drag')
assert.match(composite, /movePanelPercent/, 'composite stock move panel should keep a user-adjustable height percentage')
assert.match(composite, /MOVE_PANEL_STORAGE_KEY/, 'composite stock move panel should persist the adjusted height')
assert.match(composite, /--zt-table-flex/, 'composite table body should use a resizable flex share')
assert.match(composite, /--move-panel-flex/, 'composite stock move panel should use a resizable flex share')
assert.match(composite, /cursor:\s*row-resize/, 'resize handle should use vertical resize cursor')
assert.match(composite, /STOCK_MOVE_CACHE_TTL_MS/, 'composite stock move panel should cache arbitrary TDX-linked stock analysis')
assert.match(composite, /readCachedStockMove/, 'composite stock move panel should reuse cached stock analysis immediately')
assert.doesNotMatch(composite, />\s*异动解析\s*</, 'composite stock move panel should not show a redundant analysis label')
assert.doesNotMatch(composite, /点击股票查看异动解析|异动解析加载/, 'composite stock move panel should not use verbose analysis placeholder text')
assert.doesNotMatch(composite, /stockMoveLoading[\s\S]{0,120}>加载中|查询中/, 'composite stock move panel should not show a query/loading state')
